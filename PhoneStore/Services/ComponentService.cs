    ...
    public class ComponentService : EntityCrudService<Component, ComponentFilter>
    {
        ...
        protected override IQueryable<Component> ApplyEntityFilter(IQueryable<Component> query, ComponentFilter filter)
        {
            ...
            if (!string.IsNullOrWhiteSpace(filter.Title?.Value))
            {
                ...
                {
                    case "contains":
                        query = query.Where(c => c.Title.ToLower().Contains(titleValue));
                        break;
                    case "startswith":
                        query = query.Where(c => c.Title.ToLower().StartsWith(titleValue));
                        break;
                    case "endswith":
                        query = query.Where(c => c.Title.ToLower().EndsWith(titleValue));
                        break;
                    default:
                        query = query.Where(c => c.Title.ToLower() == titleValue);
                        break;
                }
            }
